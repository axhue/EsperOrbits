<template>
  ...
  <form v-on:submit.prevent="startTimer()">
    ...
    <div v-if="timer_on">
    <div class="sub-main">
      <button class="timerButton" v-on:click="Complete(false)" v-if="timer_on">Complete</button>
    </div>
    <div class="sub-main">
      <button class="timerButton" v-on:click="Complete(true)" v-if="timer_on">Close</button>
    </div>
    <div class="sub-main">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions(['createNode', 'updateSync']),
      startTimer () {
      ...
      Complete (close = false) {
        ...
        this.nodecount += 1
        let sync = this.$store.state.sync
        let newsync = sync + this.count / 60
        this.updateSync({sync: newsync})
        this.resetTimer()
        if (close === false) {
          this.startTimer()
        }
